...
class ProfitTable {
	...
	addOrder(order) {
		...
		const { Trader: traderId, StkCode: stkCode, Quantity: qty, TradeType: type } = result.value;
		const { Price: price, Fee: fee } = order;
		// const { Trader: traderId, StkCode: stkCode, Quantity: qty, Price: price, TradeType: type, Fee: fee } = order;

		const trader = this.findTrader(traderId);

		if (type === 'Buy')
			trader.buy(stkCode, qty, price, fee);
		else if (type === 'Sell')
			trader.sell(stkCode, qty, price, fee);
		else
